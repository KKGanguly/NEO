...
class EZROptimizer(BaseOptimizer):
    ...
    def optimize(self):
        ...
        print(f"=== Running EZR (BL Active Learning, acq=near, budget={n_trials}) ===")

        # 1) Set BL stopping condition
        bl.the.Stop = n_trials

        # 2) EZR = use near acquisition
        bl.the.acq = "near"

        # 3) Run BL Active Learning
        result = bl.actLearn(self.bl_data, shuffle=True)

        # 4) Best row from BL
        best_row = bl.first(result.best.rows)

        # 5) Extract only X columns
        x_len = len(self.bl_data.cols.x)
        best_hp = dict(zip(self.bl_data.cols.names[:x_len], best_row[:x_len]))

        # 6) Evaluate using BL’s internal distance-to-heaven metric
